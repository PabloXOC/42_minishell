...
/*   Updated: 2024/05/29 15:09:50 by ffauth-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_fill_nodes(t_clist **node, t_clist **head,
	char **command, char *path)
{
	if (command == NULL || path == NULL)
	{
		if (*head != NULL)
			ft_t_clist_clear(head);
		return (ERROR);
	}
	if (*head != NULL)
	{
		*node = ft_new_t_clist(command, path);
		ft_t_clist_add_back(head, *node);
	}
	if (*head == NULL)
		*head = ft_new_t_clist(command, path);
	return (OK);
}

t_clist	*ft_fill_commands(char **argv, char **envp, int argc)
{
	t_clist	*node;
	t_clist	*head;
	int		pos;
